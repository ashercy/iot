...
bool DFRobot_SIM7000::checkSIMStatus(void)
{
  ...
  while(count < 3){
    if(checkSendCmd("AT+CPIN?\r\n","READY")){
      break;
    }else if(checkSendCmd("AT+CPIN?\r\n","SIM PIN")){
      sendCmd("AT+CPIN=0000\r\n");
    }else{
      count++;
      delay(300);
    }
...
bool DFRobot_SIM7000::mqttConnect(const char* iot_client, const char* iot_username, const char* iot_key)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    ...
    // char MQTThead[10]={0x00,0x04,0x4d,0x51,0x54,0x54,0x04,(char)0x02,0x02,(char)0x58};
    char MQTTbuff[50]={0};
    MQTTbuff[0] = 0x10;
    sendBuff(MQTTbuff,1);
    int leng = 10;
...
bool DFRobot_SIM7000::mqttPublish(const char* iot_topic, const char* iot_data)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    DBG("aa");
    char mybuff[50] = {0x30};
    int next = 4;
    // int len = strlen(iot_topic)+iot_data.length()+2;
    int len = strlen(iot_topic)+strlen(iot_data)+2;
    mybuff[1] = (char)len;
    mybuff[2] = (char)(strlen(iot_topic)/256);
    mybuff[3] = (char)(strlen(iot_topic)%256);
    // sendBuff(mybuff,4);
    for(int i=0;i<strlen(iot_topic);i++){
      mybuff[i+next] = iot_topic[i];
    }
    next += strlen(iot_topic);
    // for(int i=0;i<iot_data.length();i++){
    for(int i=0;i<strlen(iot_data);i++){
      mybuff[next+i] = iot_data[i];
    }
    // next += iot_data.length();
    next += strlen(iot_data);
    mybuff[next++] = (char)0x1a;
    sendBuff(mybuff,next);
    if(checkSendCmd("","CLOSED")){
      return false;
    }else{
      return true;
...
bool DFRobot_SIM7000::mqttCONNandPUB(const char* iot_client, const char* iot_username, const char* iot_key,const char* iot_topic, const char* iot_data)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    char MQTThead[10]={0x00,0x04,0x4d,0x51,0x54,0x54,0x04,(char)0xC2,0x0b,(char)0xb8};
    char MQTTbuff[50]={0};
...
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    char MQTTbuff[50]={(char)0x82};
    // MQTTbuff[0] = (char)0x82;
    MQTTbuff[1] = (char)strlen(iot_topic)+5;
    MQTTbuff[2] = (char)0x00;
    MQTTbuff[3] = (char)0x01;
    MQTTbuff[4] = (char)(strlen(iot_topic)/256);
    MQTTbuff[5] = (char)(strlen(iot_topic)%256);
    int next = 6;
    for(int i=0;i<strlen(iot_topic);i++){
      MQTTbuff[next+i] = iot_topic[i];
    }
    next += strlen(iot_topic);
    MQTTbuff[next++] = (char)0x01; //QoS
    MQTTbuff[next++] = (char)0x1a;
    sendBuff(MQTTbuff,next);
    if(checkSendCmd("","CLOSED")){
      return false;
    }else{
      return true;
